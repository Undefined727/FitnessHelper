...
function draw() {
  ...
  if (poses && poses.length > 0) {
    let jumpingJackString = `Jumping Jacks completed: ${reps}`;
    text(jumpingJackString, 100, 90);
  }
  else {
...
function inPosition() {
  ...
  leftFoot = poses[0].keypoints[15];
  rightShoulder = poses[0].keypoints[6];
  rightElbow = poses[0].keypoints[8];
  rightFoot = poses[0].keypoints[16];


  if (leftElbow.y > leftShoulder.y && rightElbow.y > rightShoulder.y && leftFoot.x < leftShoulder.x && rightFoot.x > rightShoulder.x && downPosition == true) {
    var msg = new SpeechSynthesisUtterance(str(reps+1));
    window.speechSynthesis.speak(msg);
    reps = reps + 1;
    downPosition = false;
  } else if (leftElbow.y < leftShoulder.y && rightElbow.y < rightShoulder.y && leftFoot.x > leftShoulder.x && rightFoot.x < rightShoulder.x && downPosition == false) {
